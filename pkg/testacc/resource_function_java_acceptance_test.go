...
func TestAcc_FunctionJava_StagedBasic(t *testing.T) {
	...
	tmpJavaFunction := testClient().CreateSampleJavaFunctionAndJarOnStage(t, stage)
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			// change type externally by creating a new function with the exact same id but using different language
			{
				PreConfig: func() {
					testClient().Function.DropFunctionFunc(t, id)()
					testClient().Function.CreateScalaStaged(t, id, dataType, tmpJavaFunction.JarLocation(), handler)
